...
import { 
  ...
  FaDownload, FaUpload, FaChevronLeft, FaChevronRight, FaPrint, FaUser 
} from 'react-icons/fa';

// 🔹 Стили модального окна (лучше вынести в CSS, но для скорости оставим тут)
const MODAL_STYLES = {
  overlay: {
  ...
  content: {
    background: '#fff', padding: '24px', borderRadius: '12px', width: '340px',
    boxShadow: '0 10px 30px rgba(0,0,0,0.25)', textAlign: 'center', position: 'relative'
  },
          ...
          <div className="control-group modern-select-wrapper">
            <select onChange={(e) => { if (e.target.value) { onRoadGroupAdd(e.target.value); e.target.value = ''; } }} className="modern-select" disabled={loadedRoadGroups.length >= roadGroups.length}>
              ...
              {roadGroups.filter(g => !loadedRoadGroups.includes(g)).map(group => (
                <option key={group} value={group}>{group.replace('.geojson', '')}</option>
              ))}
            </select>
            <div className="select-arrow">+</div>
          </div>
          ...
          <div className="control-group modern-select-wrapper">
            <select onChange={(e) => { if (e.target.value) { onGroupAdd(e.target.value); e.target.value = ''; } }} className="modern-select" disabled={loadedGroups.length >= groups.length}>
              ...
              {groups.filter(g => !loadedGroups.includes(g)).map(group => (
                <option key={group} value={group}>{group.replace('.json', '')}</option>
              ))}
            </select>
            <div className="select-arrow">+</div>
          </div>
          ...
          <div className="control-group modern-dropdown-wrapper" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}>
            ...
            {isDropdownOpen && (
              <div className="dropdown-panel-content">
                <label className="modern-btn secondary file-upload">
                  <span className="btn-icon"><FaUpload /></span>
                  <span className="btn-text">Импорт точек</span>
                  <input type="file" accept=".geojson,.json" onChange={handlePointsGeoJSONUpload} className="file-input" />
                </label>
                <label className="modern-btn secondary file-upload">
                  <span className="btn-icon"><FaUpload /></span>
                  <span className="btn-text">Импорт маршрутов</span>
                  <input type="file" accept=".geojson,.json" onChange={handleGeoJSONUpload} className="file-input" />
                </label>
                <button className="modern-btn secondary" onClick={downloadGeoJSON}>
                  <span className="btn-icon"><FaDownload /></span>
                  <span className="btn-text">Экспорт маршрутов</span>
                </button>
                <button className="modern-btn danger" onClick={clearRoutes}>
                  <span className="btn-icon"><FaTrashAlt /></span>
                  <span className="btn-text">Очистить маршруты</span>
                </button>
                <button className="modern-btn secondary" onClick={openPrintModal}>
                  <span className="btn-icon"><FaPrint /></span>
                  <span className="btn-text">Печать / Экспорт</span>
                </button>
              </div>
            )}
          </div>
        ...
        <div style={MODAL_STYLES.overlay} onClick={closePrintModal}>
          <div style={MODAL_STYLES.content} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginBottom: '16px' }}>
              {['A4', 'A0'].map(fmt => (
                <button
                  key={fmt}
                  style={{
                    padding: '10px 24px', borderRadius: '8px', border: `2px solid ${selectedFormat === fmt ? '#4CAF50' : '#e5e7eb'}`,
                    background: selectedFormat === fmt ? '#4CAF50' : '#f9fafb', color: selectedFormat === fmt ? '#fff' : '#374151',
            ...
            <div style={MODAL_STYLES.btnGroup}>
              <button onClick={handlePrintDownload} style={{ padding: '10px 20px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>
                Скачать .doc
              </button>
              <button onClick={closePrintModal} style={{ padding: '10px 20px', background: '#f3f4f6', color: '#374151', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
                Отмена
              </button>
            </div>
          </div>
...
// ✅ Исправленные вспомогательные компоненты (без синтаксических ошибок)
