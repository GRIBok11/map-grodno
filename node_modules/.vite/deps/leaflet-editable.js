...
import { require_leaflet_src } from "./leaflet-src-CdxAvIfg.js";

//#region node_modules/leaflet-editable/src/Leaflet.Editable.js
var require_Leaflet_Editable = /* @__PURE__ */ __commonJS({ "node_modules/leaflet-editable/src/Leaflet.Editable.js": ((exports, module) => {
	((factory, window$1) => {
		...
		if (typeof window$1 !== "undefined" && window$1.L) factory(window$1.L);
	})((L) => {
		...
		L.extend(L.Editable, { makeCancellable: (e) => {
			e.cancel = () => {
				e._cancelled = true;
			};
		} });
		...
		L.Map.addInitHook(function() {
			this.whenReady(function() {
				if (this.options.editable) this.editTools = new this.options.editToolsClass(this, this.options.editOptions);
			});
		});
		...
		L.Editable.PathEditor = L.Editable.BaseEditor.extend({
			...
			removeHooks: function() {
				...
				if (this.feature) this.map.off("moveend", this.onMoveEnd, this);
			},
			onMoveEnd: function() {
				this.initVertexMarkers();
			},
			initVertexMarkers: function(latlngs) {
				...
				else for (const member of latlngs) this.initVertexMarkers(member);
			},
			getLatLngs: function() {
			...
			refreshVertexMarkers: function(latlngs) {
				...
				for (const latlng of latlngs) latlng.__vertex.update();
			},
			...
			onVertexMarkerClick: function(e) {
				...
				let commit;
				if (e.originalEvent.ctrlKey) this.onVertexMarkerCtrlClick(e);
				else if (e.originalEvent.altKey) this.onVertexMarkerAltClick(e);
				else if (e.originalEvent.shiftKey) this.onVertexMarkerShiftClick(e);
				else if (e.originalEvent.metaKey) this.onVertexMarkerMetaKeyClick(e);
				else if (index === e.vertex.getLastIndex() && this._drawing === L.Editable.FORWARD) {
					if (index >= this.MIN_VERTEX - 1) commit = true;
				} else if (index === 0 && this._drawing === L.Editable.BACKWARD && this._drawnLatLngs.length >= this.MIN_VERTEX) commit = true;
				else if (index === 0 && this._drawing === L.Editable.FORWARD && this._drawnLatLngs.length >= this.MIN_VERTEX && this.CLOSED) commit = true;
				else this.onVertexRawMarkerClick(e);
			...
			_deleteShape: function(shape, latlngs) {
				...
				const inplaceDelete = (latlngs$1, shape$1) => {
					return latlngs$1.splice(0, Number.MAX_VALUE);
				};
				const spliceDelete = (latlngs$1, shape$1) => {
					latlngs$1.splice(latlngs$1.indexOf(shape$1), 1);
					if (!latlngs$1.length) this._deleteShape(latlngs$1);
					return shape$1;
				};
		...
		L.Editable.PolygonEditor = L.Editable.PathEditor.extend({
			...
			ensureMulti: function() {
				if (this.feature._latlngs.length && isFlat(this.feature._latlngs[0])) this.feature._latlngs = [this.feature._latlngs];
			},
			...
			vertexCanBeDeleted: function(vertex) {
				...
				if (L.Util.indexOf(parent, vertex.latlngs) > 0) return true;
		...
		L.Editable.CircleEditor = L.Editable.PathEditor.extend({
			...
			resize: function(e) {
				let radius;
				if (this.map.options.crs) radius = this.map.options.crs.distance(this.feature._latlng, e.latlng);
				else radius = this.feature._latlng.distanceTo(e.latlng);
		...
		const EditableMixin = {
			createEditor: function(map) {
				...
				return new (this.options.editorClass || (this.getEditorClass(tools)))(map, this, this.options.editOptions);
			},
		...
		const PolygonMixin = {
			...
			shapeAt: function(latlng, latlngs) {
				...
				if (!latlngs.length) return shape;
				if (isFlat(latlngs) && this.isInLatLngs(latlng, latlngs)) shape = latlngs;
				if (isFlat(latlngs[0]) && this.isInLatLngs(latlng, latlngs[0])) shape = latlngs;
				else for (const member of latlngs) if (this.isInLatLngs(latlng, member[0])) return member;
				return shape;
			},
